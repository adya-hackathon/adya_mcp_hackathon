...
export class CloudinaryAuthHook
    ...
    async beforeRequest(hookCtx: BeforeRequestContext, request: Request): Promise<Request> {
        ...
        if (securityVal) {
            ...
            // Handle MCP client credential format (__credentials__)
            else if (securityVal.__credentials__) {
                if (securityVal.__credentials__.apiKey) {
                    apiKey = securityVal.__credentials__.apiKey;
                }
                if (securityVal.__credentials__.apiSecret) {
                    apiSecret = securityVal.__credentials__.apiSecret;
                }
            }
        }
        if (!apiKey || !apiSecret) {
