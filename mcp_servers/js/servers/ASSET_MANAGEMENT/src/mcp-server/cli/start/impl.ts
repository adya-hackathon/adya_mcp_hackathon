...
export async function main(this: LocalContext, flags: StartCommandFlags) {
  // Try to load credentials from hackathon-style config if present
  let loadedCreds = null;
  try {
    const credPath = path.resolve(process.cwd(), "credentials.json");
    if (fs.existsSync(credPath)) {
      const credsRaw = fs.readFileSync(credPath, "utf-8");
      const creds = JSON.parse(credsRaw);
      if (creds["ASSET_MANAGEMENT"]) {
        loadedCreds = creds["ASSET_MANAGEMENT"];
      }
    }
  } catch (e) {
    // Ignore errors, fallback to flags
  }

  // Prefer loaded credentials, fallback to CLI flags
  const apiKey = loadedCreds?.apiKey ?? flags["api-key"];
  const apiSecret = loadedCreds?.apiSecret ?? flags["api-secret"];
  const cloudName = loadedCreds?.cloudName ?? flags["cloud-name"];
  ...
  switch (flags.transport) {
    case "stdio":
      await startStdio({ ...flags, "api-key": apiKey, "api-secret": apiSecret, "cloud-name": cloudName });
      break;
    case "sse":
      await startSSE({ ...flags, "api-key": apiKey, "api-secret": apiSecret, "cloud-name": cloudName });
      break;
    default:
